...
hypernames=["none"]
for x in hypernetworks.keys(): hypernames.append(x[:x.find('(')])
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        def showxy(type):
            ...
            vvisible = [False,False,False,False,False,False]
            if axis_options[type].label == "Seed":
                vvisible[0]=True
                vvisible[1]=True
            elif axis_options[type].label == "Checkpoint name":
                vvisible[2]=True
            elif axis_options[type].label == "Sampler":
                vvisible[3]=True
            elif axis_options[type].label == "VAE":
                vvisible[4]=True
            elif axis_options[type].label == "Hypernetwork":
                vvisible[5]=True
            else:
                vvisible[0]=True
            grs=[gr.update(visible=vvisible[0])]
            for x in range(len(vvisible)-1):
                if x+1 ==1:
                     grs.append(gr.update(visible=vvisible[x+1],value=0))
                else:
                     grs.append(gr.update(visible=vvisible[x+1],value=[]))
            return grs

        x_type.change(
            fn=showxy,
            inputs=[x_type],
            outputs=[x_values,x_randseednum,x_checkpoints,x_samplers,x_vaes,x_hnets]
        )

        y_type.change(
            fn=showxy,
            inputs=[y_type],
            outputs=[y_values,y_randseednum,y_checkpoints,y_samplers,y_vaes,y_hnets]
        )

        x_samplers.change(fn=lambda x:",".join(x),inputs=[x_samplers],outputs=[x_values])
        x_checkpoints.change(fn=lambda x:",".join(x),inputs=[x_checkpoints],outputs=[x_values])
        x_vaes.change(fn=lambda x:",".join(x),inputs=[x_vaes],outputs=[x_values])
        x_hnets.change(fn=lambda x:",".join(x),inputs=[x_hnets],outputs=[x_values])

        y_samplers.change(fn=lambda x:",".join(x),inputs=[y_samplers],outputs=[y_values])
        y_checkpoints.change(fn=lambda x:",".join(x),inputs=[y_checkpoints],outputs=[y_values])
        y_vaes.change(fn=lambda x:",".join(x),inputs=[y_vaes],outputs=[y_values])
        y_hnets.change(fn=lambda x:",".join(x),inputs=[y_hnets],outputs=[y_values])
        ...
        x_randseednum.change(fn=makerand,inputs=[x_randseednum],outputs=[x_values])
        y_randseednum.change(fn=makerand,inputs=[y_randseednum],outputs=[y_values])
